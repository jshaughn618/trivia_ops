...
export function EditionDetailPage() {
  ...
  const [speedRoundAudioError, setSpeedRoundAudioError] = useState<string | null>(null);
  ...
  const buildItemDraftFromItem = (item: EditionItem): ItemDraft => {
    ...
    const isStandardTextItem = !isMusic && gameTypeId !== 'audio' && questionType !== 'multiple_choice';
    const primaryAnswer = safeString(parsedAnswerParts[0]?.answer ?? item.answer);
    const answerPartsDraft =
      gameTypeId === 'audio'
        ...
        : isMusic && parsedAnswerParts.length === 1
          ? parsedAnswerParts
          : isStandardTextItem && parsedAnswerParts.length > 1
            ? parsedAnswerParts
          : [];
    return {
  ...
  const updateDraftPrimaryAnswer = (draft: ItemDraft, nextAnswer: string): ItemDraft => {
    if (gameTypeId !== 'music' && gameTypeId !== 'audio' && draft.question_type !== 'multiple_choice' && draft.answer_parts.length > 0) {
      const nextParts = [...draft.answer_parts];
      const first = nextParts[0] ?? { label: 'Answer', answer: '', points: draft.answer_a_points };
      nextParts[0] = {
        ...first,
        label: safeTrim(first.label) || 'Answer',
        answer: nextAnswer
      };
      return {
        ...draft,
  ...
  const handleCreateItem = async () => {
    ...
    let standardAnswerA: string | null = null;
  ...
  const uploadSpeedRoundAudio = async (file: File) => {
    if (!editionId || !edition) return;
    const isMp3 = file.type === 'audio/mpeg' || file.name.toLowerCase().endsWith('.mp3');
    if (!isMp3) {
      setSpeedRoundAudioError('MP3 files only.');
      return;
    }
    setSpeedRoundAudioUploading(true);
    setSpeedRoundAudioError(null);

    const uploadRes = await api.uploadMedia(file, 'audio');
    if (!uploadRes.ok) {
      setSpeedRoundAudioUploading(false);
      setSpeedRoundAudioError(formatApiError(uploadRes, 'Upload failed.'));
      return;
    }

    const previousKey = edition.speed_round_audio_key;
    const updateRes = await api.updateEdition(editionId, {
      speed_round_audio_key: uploadRes.data.key,
      speed_round_audio_name: file.name
    });
    setSpeedRoundAudioUploading(false);
    if (!updateRes.ok) {
      await api.deleteMedia(uploadRes.data.key);
      setSpeedRoundAudioError(formatApiError(updateRes, 'Failed to save speed round audio.'));
      return;
    }
  ...
  const removeSpeedRoundAudio = async () => {
    if (!editionId || !edition?.speed_round_audio_key) return;
    setSpeedRoundAudioUploading(true);
    setSpeedRoundAudioError(null);
    const previousKey = edition.speed_round_audio_key;
    const updateRes = await api.updateEdition(editionId, {
      speed_round_audio_key: null,
      speed_round_audio_name: null
    });
    setSpeedRoundAudioUploading(false);
    if (!updateRes.ok) {
      setSpeedRoundAudioError(formatApiError(updateRes, 'Failed to remove speed round audio.'));
      return;
    }
    setEdition(updateRes.data);
    await api.deleteMedia(previousKey);
  ...
  const generateAnswer = async (overridePrompt?: string) => {
    ...
    setItemDraft((draft) => updateDraftPrimaryAnswer(draft, safeTrim(line)));
  };
  ...
  const renderEditPanel = (item: EditionItem, index: number) => (
    <div className="editor-form border-2 border-border bg-panel p-3">
      <div className="text-xs font-display uppercase tracking-[0.3em] text-muted">
        {item.question_type === 'tiebreaker' ? 'Edit Tiebreaker' : `Edit Item ${index >= 0 ? index + 1 : item.ordinal}`}
      </div>
      <div className="mt-3 grid gap-3">
        {gameTypeId === 'music' && (
          <label className="flex flex-col gap-2 text-xs font-display uppercase tracking-[0.25em] text-muted">
            ...
            <select
              ...
              onChange={(event) =>
                setItemDraft((draft) => {
                  const mode = event.target.value as 'audio' | 'text' | 'labeled';
                  const nextParts =
                    mode === 'audio'
            ...
            >
              ...
              <option value="labeled">Labeled Response</option>
            ...
            {gameTypeId !== 'music' && itemDraft.answer_parts.length > 0 ? (
              <div className="flex flex-col gap-2">
                ...
                <div className="flex flex-col gap-2">
                  {itemDraft.answer_parts.map((part, idx) => (
                    <div key={`standard-answer-part-edit-${idx}`} className="grid gap-2 sm:grid-cols-[1fr,2fr,120px,auto] sm:items-end">
                        ...
                        <input
                          ...
                          onChange={(event) =>
                            setItemDraft((draft) => {
                              ...
                              return { ...draft, answer_parts: next, answer: idx === 0 ? event.target.value : draft.answer };
                            })
                          }
                        />
                      </label>
                          ...
                          onChange={(event) =>
                            setItemDraft((draft) => {
                              ...
                              return {
                                ...draft,
                                answer_parts: next,
                                answer_a_points: idx === 0 ? Number(event.target.value) : draft.answer_a_points
                              };
                            })
                          }
                        />
                ...
                <div className="flex flex-wrap gap-2">
                  ...
                  <SecondaryButton
                    onClick={() =>
                      setItemDraft((draft) => ({
                        ...draft,
                        answer: draft.answer_parts[0]?.answer ?? draft.answer,
                        answer_a_points: draft.answer_parts[0]?.points ?? draft.answer_a_points,
                        answer_parts: []
                      }))
                    }
                    className="px-3 py-2 text-xs"
              ...
              <label className="flex flex-col gap-2 text-xs font-display uppercase tracking-[0.25em] text-muted">
                ...
                <input
                  ...
                  onChange={(event) => setItemDraft((draft) => updateDraftPrimaryAnswer(draft, event.target.value))}
                />
                {gameTypeId !== 'music' && (
                  <div className="grid gap-2 sm:grid-cols-[160px,auto] sm:items-end">
                    <label className="flex flex-col gap-1 text-[10px] font-display uppercase tracking-[0.25em] text-muted">
                      Points
                      <input
                        className="h-10 px-3"
                    ...
                    <SecondaryButton
                      onClick={() =>
                        setItemDraft((draft) => ({
                          ...draft,
                          answer_parts: [{ label: 'Answer', answer: draft.answer, points: draft.answer_a_points }]
                        }))
                      }
        ...
        {gameTypeId === 'music' && itemDraft.item_mode !== 'text' && (
          <div className="flex flex-col gap-2">
            <div className="text-xs font-display uppercase tracking-[0.3em] text-muted">
              {itemDraft.item_mode === 'labeled' ? 'Response' : 'Answer parts'}
            </div>
            <div className="flex flex-col gap-2">
              {itemDraft.answer_parts.map((part, idx) => (
                <div
        ...
        {gameTypeId === 'music' && itemDraft.item_mode === 'audio' && isMusicSpeedRound && (
          <div className="rounded-md border border-border bg-panel2 px-3 py-2 text-[10px] uppercase tracking-[0.2em] text-muted">
            Speed round audio is managed at the edition level.
          </div>
        )}
        {itemValidationError && (
          ...
          <>
            {gameTypeId !== 'music' && itemDraft.answer_parts.length > 0 ? (
              <div className="flex flex-col gap-2">
                ...
                <div className="flex flex-col gap-2">
                  {itemDraft.answer_parts.map((part, idx) => (
                    <div key={`standard-answer-part-new-${idx}`} className="grid gap-2 sm:grid-cols-[1fr,2fr,120px,auto] sm:items-end">
                      <label className="flex flex-col gap-1 text-[10px] font-display uppercase tracking-[0.25em] text-muted">
                        Label
                        <input
                          className="h-10 px-3"
              ...
              {usesSpeedRoundSetup ? (
                <div className="mt-3 flex flex-col gap-3">
                  ...
                  <div className="rounded-md border border-border bg-panel p-3">
                    <div className="text-xs font-display uppercase tracking-[0.2em] text-muted">Edition speed round audio</div>
                    <div className="mt-2 flex flex-wrap items-center gap-2">
                      <input
                        ref={speedRoundAudioRef}
                        type="file"
                        accept="audio/mpeg,audio/mp3"
                        className="hidden"
                        onChange={(event) => {
                          const file = event.target.files?.[0];
                          event.target.value = '';
                          if (file) uploadSpeedRoundAudio(file);
                        }}
                      />
                      <button
                        type="button"
                        onClick={() => speedRoundAudioRef.current?.click()}
                        className="border-2 border-border px-3 py-1 text-[10px] font-display uppercase tracking-[0.3em] text-muted hover:border-accent-ink hover:text-text"
                        disabled={speedRoundAudioUploading}
                      >
                        {speedRoundAudioUploading ? 'Uploading' : 'Upload MP3'}
                      </button>
                      {edition?.speed_round_audio_key && (
                        <button
                          type="button"
                          onClick={removeSpeedRoundAudio}
                          className="border-2 border-danger px-3 py-1 text-[10px] font-display uppercase tracking-[0.3em] text-danger hover:border-[#9d2a24]"
                          disabled={speedRoundAudioUploading}
                        >
                          Remove
                      ...
                      {edition?.speed_round_audio_name && (
                        <span className="text-xs text-muted">{edition.speed_round_audio_name}</span>
                      )}
                      {!edition?.speed_round_audio_key && !speedRoundAudioUploading && (
                        <span className="text-xs text-muted">No clip uploaded.</span>
                      )}
                    </div>
                    {edition?.speed_round_audio_key && (
                      <div className="mt-2">
                        <audio className="w-full" controls src={api.mediaUrl(edition.speed_round_audio_key)} />
                      </div>
                    )}
                    {speedRoundAudioError && (
                      <div className="mt-2 text-xs text-danger">{speedRoundAudioError}</div>
                    )}
                  </div>
                  <label className="flex flex-col gap-2 text-xs font-display uppercase tracking-[0.25em] text-muted">
                    Round answers
                ...
                {orderedItems.map((item, index) => (
                  <div key={item.id} className="flex flex-col gap-3">
                    ...
                    >
                      <div className="flex items-center justify-between">
                        <div className="text-xs font-display uppercase tracking-[0.3em] text-muted">
                          {item.question_type === 'tiebreaker' ? 'TIEBREAKER' : `Item ${index + 1}`}
                        </div>
                      ...
                      <div className="mt-2 text-sm text-text">
                        {safeTrim(item.prompt)
                          ...
                          : gameTypeId === 'music' && item.media_type === 'audio'
                            ...
                            : gameTypeId === 'music' && parseAnswerPartsJson(item.answer_parts_json ?? null, item).length === 1
                              ? parseAnswerPartsJson(item.answer_parts_json ?? null, item)[0]?.label ?? ''
                            : ''}
                      </div>
                      ...
                      {gameTypeId === 'music' && (
                        <div className="mt-2 text-[10px] uppercase tracking-[0.2em] text-muted">
                          {isMusicSpeedRound
                            ? edition?.speed_round_audio_key ? 'Edition speed round audio attached' : 'No edition speed round audio'
