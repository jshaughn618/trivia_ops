...
const resolveScoresheetIncludeOptions = (event?: Event | null): ScoresheetIncludeOptions => ({
  ...
  logo: Number(event?.include_scoresheet_logo ?? 1) === 1,
  locationLogo: Number(event?.include_scoresheet_location_logo ?? 0) === 1
});

const parseScoresheetSpecialCheckboxes = (value?: string | null): ScoresheetSpecialCheckbox[] => {
  if (!value) return [];
  try {
    ...
    return parsed
      .map((item) => {
        ...
        return { header, detail: detail || null };
      })
      .filter((item): item is ScoresheetSpecialCheckbox => Boolean(item));
  } catch {
    return [];
  }
};
...
const drawPageHeader = (
  ...
  locationName: string,
  fonts: { regular: any; bold: any },
  options?: { showEventCode?: boolean; includeOptions?: ScoresheetIncludeOptions; locationLogoImage?: any }
) => {
  const titleSize = 14;
  const metaSize = 9;
  const headerTop = PAGE_HEIGHT - PAGE_MARGIN;
  ...
  if (includeOptions.locationLogo && options?.locationLogoImage) {
    const maxLogoWidth = 120;
    const maxLogoHeight = 40;
    const scale = Math.min(
      maxLogoWidth / options.locationLogoImage.width,
      maxLogoHeight / options.locationLogoImage.height,
      1
    );
    locationLogoWidth = options.locationLogoImage.width * scale;
    const locationLogoHeight = options.locationLogoImage.height * scale;
    page.drawImage(options.locationLogoImage, {
      x: PAGE_WIDTH - PAGE_MARGIN - locationLogoWidth,
      y: headerTop - locationLogoHeight,
      width: locationLogoWidth,
      height: locationLogoHeight
    });
  }
  const headerTextMaxWidth = PAGE_WIDTH - PAGE_MARGIN * 2 - (locationLogoWidth > 0 ? locationLogoWidth + 16 : 0);
  ...
  if (includeOptions.eventName) {
    drawPdfText(page, truncateText(fonts.bold, event.title, headerTextMaxWidth, titleSize), {
      x: PAGE_MARGIN,
      y: cursorY,
      size: titleSize,
      font: fonts.bold
  ...
  if (metaLine) {
    drawPdfText(page, truncateText(fonts.regular, metaLine, headerTextMaxWidth, metaSize), {
      x: PAGE_MARGIN,
      y: cursorY,
      size: metaSize,
      font: fonts.regular
  ...
  if ((options?.showEventCode ?? false) && includeOptions.eventCode && event.public_code) {
    ...
    drawPdfText(page, truncateText(fonts.regular, codeText, headerTextMaxWidth, metaSize), {
      x: PAGE_MARGIN,
      y: cursorY,
      size: metaSize,
      font: fonts.regular
...
const renderTeamBlock = (
  ...
  extras: {
    ...
    teamName?: string;
    teamPlaceholder?: boolean;
    specialCheckboxes?: ScoresheetSpecialCheckbox[];
    includeOptions?: ScoresheetIncludeOptions;
  }
) => {
  ...
  const specialCheckboxes = extras.specialCheckboxes ?? [];
  if (specialCheckboxes.length > 0) {
    ...
    specialCheckboxes.forEach((item) => {
      if (cursorY <= cell.y + padding + boxSize) return;
      const headerLines = wrapPdfText(fonts.bold, item.header, textWidth, textSize);
      const detailLines = item.detail ? wrapPdfText(fonts.regular, item.detail, textWidth, detailSize) : [];
      ...
      const detailStartY = cursorY - headerLines.length * (textSize + 3) - 2;
      detailLines.forEach((line, index) => {
        drawPdfText(page, line, {
          x: textX,
          y: detailStartY - detailSize - index * (detailSize + 2),
          size: detailSize,
          font: fonts.regular
        });
      });
      const blockHeight = Math.max(
        boxSize,
        headerLines.length * (textSize + 3) + detailLines.length * (detailSize + 2)
      );
...
const buildPdf = async (
  ...
  let locationLogoImage: any | null = null;
  ...
  if (extras?.locationLogoBytes) {
    try {
      locationLogoImage = await pdfDoc.embedPng(extras.locationLogoBytes);
    } catch {
      locationLogoImage = null;
    }
  }
  if (mode === 'scoresheet') {
    try {
  ...
  const createPage = (
    ...
    if (drawHeader) {
      drawPageHeader(page, event, locationName, fonts, {
        showEventCode,
        includeOptions,
        locationLogoImage: showLocationLogo ? locationLogoImage ?? undefined : undefined
      });
    }
  ...
  if (useMusicLayout) {
    const drawMusicHeader = (page: any) =>
      drawMusicScoresheetHeader(page, event, fonts, {
        ...
        locationLogoImage: locationLogoImage ?? undefined,
        ...
        showLocationLogo: mode === 'scoresheet' && pageCount() === 1,
        includeOptions
      });
    ...
    let pageIndex = -1;
    let page = createPage(true, 'quad', true, includeOptions.locationLogo);
    pageIndex = 0;
    let positionIndex = 0;
    ...
    const firstPage = pdfDoc.getPages()[0] ?? createPage(true, 'quad', true, includeOptions.locationLogo);
      renderTeamBlock(firstPage, getCell(0), fonts, {
        ...
        specialCheckboxes: extras?.specialCheckboxes,
        includeOptions
      });
...
export function EventDetailPage() {
  ...
  const [scoresheetSpecialCheckboxes, setScoresheetSpecialCheckboxes] = useState<ScoresheetSpecialCheckbox[]>([]);
  ...
  const loadCore = async (isActive: () => boolean = () => true) => {
    ...
    if (eventRes.ok) {
      ...
      setScoresheetSpecialCheckboxes(parseScoresheetSpecialCheckboxes(eventRes.data.scoresheet_special_checkboxes_json));
  ...
  const loadBootstrap = async (isActive: () => boolean = () => true) => {
    ...
    setScoresheetSpecialCheckboxes(parseScoresheetSpecialCheckboxes(res.data.event.scoresheet_special_checkboxes_json));
  ...
  const eventSettingsDraft = useMemo(
    () => ({
      ...
      include_scoresheet_logo: scoresheetIncludeOptions.logo,
      include_scoresheet_location_logo: scoresheetIncludeOptions.locationLogo,
      scoresheet_special_checkboxes: scoresheetSpecialCheckboxes.length > 0 ? scoresheetSpecialCheckboxes : null,
    ...
    () =>
      event
        ? {
            ...
            include_scoresheet_logo: Number(event.include_scoresheet_logo ?? 1) === 1,
            include_scoresheet_location_logo: Number(event.include_scoresheet_location_logo ?? 0) === 1,
            scoresheet_special_checkboxes: parseScoresheetSpecialCheckboxes(event.scoresheet_special_checkboxes_json),
  ...
  const location = useMemo(
    () => (event?.location_id ? locations.find((item) => item.id === event.location_id) ?? null : null),
    [event?.location_id, locations]
  );

  const promptForScoresheetSpecialCheckbox = (
    initial?: ScoresheetSpecialCheckbox
  ): ScoresheetSpecialCheckbox | null => {
    const nextHeader = window.prompt('Enter checkbox header text', initial?.header ?? '');
    if (nextHeader === null) return null;
    const header = nextHeader.trim();
    if (!header) return null;
    const nextDetail = window.prompt('Enter detail text (optional)', initial?.detail ?? '');
    if (nextDetail === null) return null;
    return {
      header,
  ...
  const generateScoresheets = async () => {
    ...
    try {
      ...
      let locationLogoBytes: Uint8Array | undefined;
      if (scoresheetIncludeOptions.locationLogo && location?.logo_key) {
        try {
          const locationLogoRes = await fetch(api.mediaUrl(location.logo_key));
          if (locationLogoRes.ok) {
            const locationLogoBlob = await locationLogoRes.blob();
            const pngBytes = await blobToPngArrayBuffer(locationLogoBlob);
            locationLogoBytes = new Uint8Array(pngBytes);
          }
        } catch {
          locationLogoBytes = undefined;
        }
      }
      const scoresheetDoc = await PDFDocument.create();
      const teamsForSheets = teams.length > 0 ? teams : [null];
      const specialCheckboxes = scoresheetSpecialCheckboxes;
      for (const team of teamsForSheets) {
        let qrDataUrl: string | undefined;
        if (event.public_code && scoresheetIncludeOptions.qrCode && specialCheckboxes.length === 0) {
      ...
      <div className="surface-card space-y-3 p-4">
        ...
        <div className="grid gap-2 sm:grid-cols-2">
          ...
          <label className="flex items-center gap-2 text-sm text-muted">
            <input
              type="checkbox"
              checked={scoresheetIncludeOptions.logo}
              onChange={(event) =>
                setScoresheetIncludeOptions((prev) => ({ ...prev, logo: event.target.checked }))
              }
            />
            Trivia Ops logo
          </label>
          <label className="flex items-center gap-2 text-sm text-muted">
            <input
              type="checkbox"
              checked={scoresheetIncludeOptions.locationLogo}
              onChange={(event) =>
                setScoresheetIncludeOptions((prev) => ({ ...prev, locationLogo: event.target.checked }))
              }
            />
            Location logo
          </label>
          ...
          <div className="sm:col-span-2 rounded-lg border border-border bg-panel2/60 p-3">
            <div className="flex flex-wrap items-center justify-between gap-2">
              <div>
                <div className="text-sm font-medium text-text">Special checkboxes</div>
                <div className="mt-1 text-xs text-muted">
                  Adds one or more empty checkboxes with header and detail text in the QR code area on pub trivia scoresheets.
                </div>
              </div>
              <SecondaryButton
                ...
                type="button"
                onClick={() => {
                  const nextItem = promptForScoresheetSpecialCheckbox();
                  if (!nextItem) return;
                  setScoresheetSpecialCheckboxes((prev) => prev.concat(nextItem));
                }}
              >
                Add checkbox
              </SecondaryButton>
            ...
            <div className="mt-3 space-y-2">
              {scoresheetSpecialCheckboxes.length > 0 ? (
                scoresheetSpecialCheckboxes.map((item, index) => (
                  <div
                    key={`${item.header}-${index}`}
                    className="flex flex-wrap items-start justify-between gap-3 rounded-lg border border-border/80 bg-panel/80 p-3"
                  >
                    <div className="min-w-0 flex-1">
                      <div className="text-sm font-medium text-text">{item.header}</div>
                      {item.detail && <div className="mt-1 text-xs text-muted">{item.detail}</div>}
                    </div>
                    <div className="flex flex-wrap gap-2">
                      <SecondaryButton
                        className="h-8 px-3 text-xs"
                        type="button"
                        onClick={() => {
                          const nextItem = promptForScoresheetSpecialCheckbox(item);
                          if (!nextItem) return;
                          setScoresheetSpecialCheckboxes((prev) =>
                            prev.map((current, currentIndex) => (currentIndex === index ? nextItem : current))
                          );
                        }}
                      >
                        Edit
                ...
                <div className="text-xs text-muted">No special checkboxes configured.</div>
              )}
            </div>
          </div>
        </div>
